...
struct Symbol
{
    int32_t scope; //!< Область видимости переменной.
                    //!< Число означает номер цикла в выражении
                    //!< -1 означает, что переменная не инициализирована
    Token init; //!< Токен, инициализирующий переменную.

    /**
     * Адрес символа во внутреннем представлении транслятора.
     *
     * Транслятор выделяет массив целых чисел, в котором хранятся значения
     * переменных. ind - это индекс в этом массиве.
     */
    int ind;
};
